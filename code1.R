...
# Choosen '2' After some Trial and Errors
ts1_filtered <- rollmean(ts1,2) 
ts2_filtered <- rollmean(ts2,2)
...
plot1 <- ggplot() +
         ...
         geom_point(data=sample1_final,aes(x=start,y=logRatio),colour="red",alpha=0.43)+xlab("Start")+ylab("LogRatio Signal")+ggtitle("Sample 1")

plot2 <- ggplot() +
  geom_point(data=sample2,aes(x=start,y=logRatio),colour="green") +
  geom_point(data=sample2_final,aes(x=start,y=logRatio),colour="red",alpha=0.43)+xlab("Start")+ylab("LogRatio Signal")+ggtitle("Sample 2")
